            ...
            <div>
                <div class="flex items-center gap-6 mb-1">
                    ...
                    <div class="flex items-center gap-2 ml-4">
                        ...
                        <a href="<?= BASE_URL ?>/admin/master-sheet.php" class="px-3 py-1.5 rounded-md bg-[#3F72AF] text-white text-sm font-medium flex items-center gap-2">
                           <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="8" y1="13" x2="16" y2="13"></line><line x1="8" y1="17" x2="16" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
                            Master Sheet
                        </a>
                        <a href="<?= BASE_URL ?>/admin/khidmat-requests.php" class="px-3 py-1.5 rounded-md text-[#DBE2EF] hover:text-white hover:bg-white/10 text-sm font-medium transition-all flex items-center gap-2">
...
<script src="<?= BASE_URL ?>/assets/js/audition-modal.js"></script>
    ...
    function requestCorrection(regId, fullName) {
        const modalHTML = `
            <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 animate-fadeIn" id="correction-modal">
                <div class="absolute inset-0 bg-[#112D4E]/60 backdrop-blur-sm transition-opacity" onclick="closeCorrectionModal()"></div>
                
                <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col animate-zoomIn overflow-hidden">
                    <div class="px-6 py-4 border-b border-[#DBE2EF] flex items-center justify-between bg-[#F9F7F7]">
                        <div>
                            <h3 class="text-lg font-bold text-[#112D4E]">Request Correction</h3>
                            <p class="text-xs text-[#6B7280]">For ${fullName}</p>
                        </div>
                        <button onclick="closeCorrectionModal()" class="p-2 text-[#6B7280] hover:text-[#112D4E] rounded-full hover:bg-[#DBE2EF]/50 transition-all">
                            ${ICONS.x}
                        </button>
                    </div>
                    ...
                    <div class="p-6 space-y-4 bg-white">
                        <div>
                            <label class="block text-xs font-bold text-[#112D4E] uppercase tracking-wider mb-1.5">Field to Correct</label>
                            <select id="correction-field" class="w-full rounded-xl border-[#DBE2EF] shadow-sm focus:border-[#3F72AF] focus:ring-[#3F72AF] text-sm p-3 border bg-[#F9F7F7] text-[#112D4E] font-medium transition-all">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-[#112D4E] uppercase tracking-wider mb-1.5">Message for User</label>
                            <textarea id="correction-msg" rows="3" class="w-full rounded-xl border-[#DBE2EF] shadow-sm focus:border-[#3F72AF] focus:ring-[#3F72AF] text-sm p-3 border bg-[#F9F7F7] text-[#112D4E] placeholder-[#9CA3AF] transition-all" placeholder="e.g., Please upload a clearer audio file of Azaan."></textarea>
                        </div>
                    </div>

                    <div class="px-6 py-4 border-t border-[#DBE2EF] flex justify-end bg-[#F9F7F7] gap-3">
                        <button onclick="closeCorrectionModal()" class="px-5 py-2.5 text-[#6B7280] hover:bg-white hover:text-[#112D4E] rounded-xl font-bold text-sm transition-all border border-transparent hover:border-[#DBE2EF]">
                            Cancel
                        </button>
                        <button onclick="submitCorrection(${regId})" class="px-6 py-2.5 bg-[#3F72AF] text-white rounded-xl hover:bg-[#2D5A8F] font-bold text-sm shadow-lg shadow-blue-200 transition-all active:scale-95">
                            Send Request
                        </button>
                    </div>
                </div>
    ...
    async function submitCorrection(regId) {
        ...
        if (!msg.trim()) {
            showDialog({
                variant: 'danger',
                title: 'Data Required',
                message: 'Please enter a message for the user explaining what needs to be corrected.',
                confirmLabel: 'OK'
            });
            return;
        }

        const btn = document.querySelector('#correction-modal button.bg-\\[\\#3F72AF\\]');
    ...
    async function viewAuditions(regId) {
        ...
        window.selectAudition = async (fileId, regId) => {
            showDialog({
                ...
                message: 'Are you sure you want to select this audition? This will unselect any other files for this user.',
                confirmLabel: 'Yes, Select',
                onConfirm: async () => {
                    // Show loading overlay or modify button state
