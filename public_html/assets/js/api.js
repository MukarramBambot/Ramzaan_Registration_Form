...
// API_BASE is set from PHP config.php via admin-header.php.
// Fallback to localhost for dev if not injected by PHP.
...
async function refreshAccessToken() {
  ...
  try {
    const res = await fetch(`${window.API_BASE}/api/token/refresh/`, {
      // Corrected endpoint for SimpleJWT usually
      method: "POST",
...
async function apiFetch(endpoint, options = {}) {
  ...
  // throwOnError: if false, non-OK responses are returned as-is (for manual error handling)
  const throwOnError = options.throwOnError !== false;
  if ('throwOnError' in options) delete options.throwOnError;
  ...
  if (!response.ok) {
    // If caller wants manual error handling, return response as-is
    if (!throwOnError) {
      return response;
    }
    ...
    // Throw an error with the message from the server or a generic status text
    const errorMsg =
      errorData.error ||
      errorData.message ||
      (errorData.detail &&
