...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Only enable PWA features on mobile devices and non-admin pages
    if (isMobile && !isAdminPage) {
        ...
        // 2. Handle PWA Installation
        let deferredPrompt;
        const installBar = document.getElementById('installBar');
        const installBtn = document.getElementById('installBtn');
        const closeInstall = document.getElementById('closeInstall');

        if (installBar && installBtn && closeInstall) {
            ...
            // iOS Support (Manual Instruction)
            ...
            if (isIOS && !isStandalone) {
                ...
                installBar.querySelector('span').innerHTML = 'Add to Home Screen: <span class="font-bold">Tap Share &rarr; Add to Home Screen</span>';
            }
        }
    } else {
        // If service worker exists but we shouldn't be using it (e.g., admin went to public page then admin page),
        // we could potentially unregister it, but for now we just don't re-register or show prompts.
        console.log('PWA features disabled for this session (Desktop or Admin).');
    }
});
