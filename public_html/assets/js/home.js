...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fileInput.addEventListener('change', (e) => {
        if (e.target.files) {
            ...
            // Validation: Audio MIME types and extensions
            const allowedExtensions = ['mp3', 'wav', 'aac', 'flac', 'm4a', 'ogg', 'webm'];
            const invalidFiles = newFiles.filter(f => {
                const ext = f.name.split('.').pop().toLowerCase();
                const isAudioMime = f.type.startsWith('audio/');
                return !isAudioMime || !allowedExtensions.includes(ext);
            });
            ...
            if (invalidFiles.length > 0) {
                ...
                showDialog({ 
                    ...
                    message: `Only audio files are allowed (${allowedExtensions.join(', ')}).\n\nInvalid files:\n${names}`, 
                    confirmLabel: 'OK' 
                });
                fileInput.value = ''; // Clear input
                selectedFiles = [];
    ...
    form.addEventListener('submit', async (e) => {
        ...
        xhr.onload = async function() {
            if (xhr.status === 201 || xhr.status === 200) {
                ...
                window.location.href = (window.BASE_URL || '') + '/khidmat/success.php';
            } else {
                let errorData = {};
                ...
                if (xhr.status === 400) {
                    // Check for specifically handled errors like already registered
                    if (errorData.its_number) {
                    ...
                    message = extractErrorMessages(errorData);
        ...
        xhr.onerror = async function() {
            ...
            handleFailureReset('Network connection failed. Please check your internet.');
        };
        ...
        function extractErrorMessages(data) {
            ...
            // Handle DRF style validation errors: { field: ["error1", "error2"] }
            let messages = [];
