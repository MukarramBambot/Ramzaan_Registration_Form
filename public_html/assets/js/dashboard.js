...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function renderHeader() {
        let html = `<th class="px-4 py-3 text-left text-xs font-bold text-[#112D4E] uppercase tracking-wider border-r border-[#DBE2EF] bg-[#F9F7F7] sticky left-0 z-30 min-w-[120px] sticky-col shadow-right">Date</th>`;
    ...
    function renderBody() {
        ...
        dates.forEach((date, dateIdx) => {
            ...
            // Date Cell
            let html = `<td class="px-4 py-2 text-sm font-medium text-[#112D4E] border-r border-[#DBE2EF] sticky left-0 z-10 bg-inherit transition-colors duration-150 ease-out sticky-col shadow-right">${date}</td>`;
            ...
            DUTY_COLUMNS.forEach(duty => {
                ...
                if (isEditing) {
                    ...
                    let content = user ? user.fullName : '–';
                    let lockIcon = assignment?.locked ? `<span class="mr-1">${ICONS.lock.replace('width="24"', 'width="12"').replace('height="24"', 'height="12"').replace('currentColor', '#3F72AF')}</span>` : '';
        ...
        if (assignment?.locked) {
            ...
            if (user) {
                ...
                showDialog({
                    variant: 'info',
                    title: 'Slot Locked',
                    message: 'This khidmat slot is locked and cannot be assigned. If this is an error, please contact the administrator to unlock it.',
                    confirmLabel: 'OK'
                });
            }
        } else {
