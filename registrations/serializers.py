...
class RegistrationCreateSerializer(serializers.ModelSerializer):
    ...
    # Accept list input for preference, but also tolerate a single string for backward compatibility.
    ...
    def validate_preference(self, value):
        # If clients POST a single string, coerce to list
...
class KhidmatRequestSerializer(serializers.ModelSerializer):
    """Detailed serializer for KhidmatRequest, used primarily by Admin."""
    ...
    def get_assignment_details(self, obj):
        return {
            'date': obj.assignment.duty_date,
            'namaaz': obj.assignment.get_namaaz_type_display(),
            'original_status': obj.assignment.status
        }
